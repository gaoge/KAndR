...
//生成镜像二叉树
void NodeTree::makeMirror(Node *&node){
	Node *tmp = node->left;
	node->left = node->right;
	node->right = tmp;
	if(node->left != NULL) makeMirror(node->left);
	if(node->right != NULL) makeMirror(node->right);
}
main()
{
	...
	cout<<endl;
	cout<<"***************after makeMirror"<<endl;
	nodeTree->makeMirror(nodeTree->head);
	nodeTree->print(nodeTree->head);
	cout<<endl;
